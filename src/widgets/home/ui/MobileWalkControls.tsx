...
export const MobileWalkControls = () => {
  ...
  return (
    <div className='pointer-events-auto fixed bottom-[max(1.25rem,env(safe-area-inset-bottom))] right-4 z-30 touch-none select-none sm:bottom-8 sm:right-6'>
      <button
        type='button'
        aria-label='터치 또는 드래그로 이동'
        onPointerDown={handlePointerDown}
        onPointerMove={handlePointerMove}
        onPointerUp={handlePointerEnd}
        onPointerCancel={handlePointerEnd}
        className='relative flex h-28 w-28 items-center justify-center rounded-full border border-white/20 bg-black/35 p-0 backdrop-blur-sm [--knob-x:0px] [--knob-y:0px]'
      >
